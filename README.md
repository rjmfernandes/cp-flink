...
- [CP Flink](#cp-flink)
  ...
  - [Create an application with durable storage](#create-an-application-with-durable-storage)
  - [Cleanup](#cleanup)
...
## Create an application with durable storage

Create the application:

```shell
confluent flink application create application-durable.json --environment env1 --url http://localhost:8080
```
...
```shell
confluent flink application web-ui-forward durable-example --environment env1 --port 8090 --url http://localhost:8080
```
